...
connects to your broker and announces a single **ChargeKeeper** device with fifty-four entities — battery
...
detected network, and the app's own diagnostics — including what it last did, and what its
lid-close wait and keep-awake hold are doing now. An availability topic, with a Last-Will, marks the
...
settings panel the MQTT page hosts. ChargeKeeper supplies the topic root, the fifty-four entity
