...
/// because the list and the device class that gives it meaning must never be declared apart.</remarks>
internal static class MqttEnumSensor
{
    ...
    /// <summary>The list as it is announced: the words themselves, then the literal an absent
    /// reading publishes. The sentinel has to be on the list — a receiver holding a state against it
    /// reads every gap as an error otherwise.</summary>
    public static IReadOnlyList<string> Announced(IReadOnlyList<string> words) =>
        [.. words, MqttPayload.None];

    /// <summary>One diagnostic entity over a fixed list of words.</summary>
    ...
    public static MqttSensor Of(
        string entityId, string name, string group, string icon,
